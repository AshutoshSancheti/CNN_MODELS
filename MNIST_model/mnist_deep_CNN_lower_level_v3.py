...
def batch_norm(layer, mean, variance):
    epsilon = 0.00000001
    layer = (layer - mean)/tf.sqrt(variance + epsilon)
    return layer
...
mean_conv1, var_conv1 = tf.nn.moments(h_conv1, [0])
h_conv1 = batch_norm(h_conv1, mean_conv1, var_conv1)
...
mean_conv2, var_conv2 = tf.nn.moments(h_conv2, [0])
h_conv2 = batch_norm(h_conv2, mean_conv2, var_conv2)

#3rd conv layerr before the second max_pool layer
W_conv3 = weight_variable([5, 5, 64, 64])
b_conv3 = bias_variable([64])
h_conv3 = tf.nn.relu(conv2d(h_conv2, W_conv3) + b_conv3)
mean_conv3, var_conv3 = tf.nn.moments(h_conv3, [0])
h_conv3 = batch_norm(h_conv3, mean_conv3, var_conv3)
...
mean_fc1, var_fc1 = tf.nn.moments(h_fc1, [0])
h_fc1 = batch_norm(h_fc1, mean_fc1, var_fc1)
...
def test_func(x_test, a, b, c, d, e, f, g, h, i, j):
    ...
    mean_relu1, var_relu1 = tf.nn.moments(relu1, [0])
    relu1 = batch_norm(relu1, mean_relu1, var_relu1)
    max1 = max_pool_2x2(relu1)
    #2nd conv and pool
    conv2 = conv2d(max1, c)
    relu2 = tf.nn.relu(conv2 + d)
    mean_relu2, var_relu2 = tf.nn.moments(relu2, [0])
    relu2 = batch_norm(relu2, mean_relu2, var_relu2)
    
    conv3 = conv2d(relu2, i)
    relu3 = tf.nn.relu(conv3 + j)
    mean_relu3, var_relu3 = tf.nn.moments(relu3, [0])
    relu3 = batch_norm(relu3, mean_relu3, var_relu3)
    max2 = max_pool_2x2(relu3)    
    #1st FC layer
    flat = tf.reshape(max2, [-1,7*7*64]) #shape is [1, 7*7*64]
    fc = tf.nn.relu(tf.matmul(flat, e) + f)   #output is [1, 1024] 
    mean_fc, var_fc = tf.nn.moments(fc, [0])
    fc = batch_norm(fc, mean_fc, var_fc)
